...
(function (global, factory) {
    ...
    function VennDiagram() {
        ...
        function chart(selection) {
            ...
            data.forEach(function (datum) {
                if (datum.label) {
                    labels[datum.sets] = datum.label + " (" + datum.size + ")";
                }
            });
...
if (d3 == undefined) {
    alert("Internet connection must be available for visualizations to work !")
}
...
var sets, style_option, legend, narrative, heading, page_title, show_stats

d3.json("data/venn.json", function (d) {
    ...
    window.document.title = d[0].doc_title;
    // switch display from light to dark mode
...
function readData(d){
    console.log(d)
    window.document.title = d[0].doc_title
    page_title = d[0].page_title
    // display heading above narrative
    style_option = d[0].style_option
    // display heading above narrative
    heading = d[0].heading
    // display narrative details about each data point
    narrative = d[0].narrative
    // display stats about each data point
    show_stats = d[0].show_stats
    // actual data used to generate the diagram
    sets = d[1].dataset
}
...
var render = function (d) {
    ...
    let div = d3.select("#venn").datum(d).call(chart)
    ...
    if(show_stats){
        ...
        d.forEach(function (item) {
            if (item.sets.length > 1) {
                let l = ul.append("li")
                let pc = ((item.size / sets[si].size) * 100).toFixed(0)
                let d = sets[si].size - item.size;
                let dpc = 100 - pc;

                l.html(sets[si].label + "(" + sets[si].size + ") :: " + sets[0].label + " (" + sets[0].size + ")<br>correlation : " + item.size + " (" + pc + "%) - difference: " + d + " (" + dpc + "%)")
                si++;
                i++;
            }
        })
...
// fade in the content panel
    let content_div = d3.select('.legend')
        .transition()
        .style("opacity", 1)
        .duration(2000)


    // zoom in the venn circles
    let x = window.innerWidth *.4
    let y = window.innerHeight *.4

    let circles = d3.selectAll('.venn-circle path')
        .attr("transform","translate("+x+","+y+") scale(0.0 0.0)")
        .transition()
        .delay(function(d,i){return 100 + (i * 300)})
        .duration(1000)
